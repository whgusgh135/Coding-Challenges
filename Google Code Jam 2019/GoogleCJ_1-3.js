    ...
    // } else {
        ...
        for(let i = 0; i < num.length - 1; i++) {
            if(parseInt(num[i]) == parseInt(num[i+1])) {
                ...
                gcd[i] = gcd_two_numbers(parseInt(num[i]), parseInt(num[i+1]));
                console.log(gcd[i]);
            }
        }
        ...
        let firstNum = num[0] / gcd[0];
        gcd.unshift(firstNum);
        let lastNum = num[num.length -1] / gcd[gcd.length - 1];
        gcd.push(lastNum);
        ...
        for(let i = 1; i <= alphabets.length; i++) {
            let largest = Math.max.apply(null, gcd);
            while(gcd.indexOf(largest) != -1) {
                letterArr[gcd.indexOf(largest)] = alphabets[alphabets.length - i];
                gcd[gcd.indexOf(largest)] = 0;
            }
        }

        let outputStr = "";
        for(let i = 0; i < letterArr.length; i++) {
            outputStr += letterArr[i];
        }
        
        let output = "Case #"+  ": " + outputStr;
        console.log(output);
        
        // for(let i = 1; i <= alphabets.length; i++) {
        //     let largestest = Math.max.apply(null, gcd);
        //     letterArr[gcd.indexOf(largest)] = alphabets[alphabets.length - i];
        // }
